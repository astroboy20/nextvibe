...
import { ChevronLeft, ChevronRight, Search, XCircle, Eye, Calendar } from "lucide-react";
...
export default function EventsPage() {
  ...
  // Real API fields: name, locationName, startsAt, mode, category, organizer, _count
  const filtered = events.filter((e: IAdminEvent) => {
    const q = search.toLowerCase();
    const organizer = e.organizer?.displayName ?? e.organizer?.username ?? "";
    return (
  ...
  const handleCancel = async () => {
    ...
    try {
      await cancelEvent({ id: cancelTarget.id, reason: cancelReason.trim() || undefined }).unwrap();
    ...
    <div className="space-y-6">
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-0.5">All events on the platform.</p>
        </div>
        {!isLoading && (
          <div className="flex items-center gap-2 text-sm text-muted-foreground bg-primary/5 border border-primary/10 px-3 py-1.5 rounded-lg">
            ...
            <span className="font-medium">{total.toLocaleString()} total events</span>
          </div>
        )}
      </div>
      ...
      <Card>
        <CardHeader className="flex flex-row items-center justify-between gap-4 flex-wrap">
          <div className="flex items-center gap-3">
            ...
            {!isLoading && search && (
              <span className="text-sm text-muted-foreground">({filtered.length} of {events.length})</span>
            )}
          </div>
        ...
        <CardContent>
          {isLoading ? (
            ...
            <p className="text-center text-muted-foreground py-8">Failed to load events.</p>
          ) : filtered.length === 0 ? (
            <EmptyState
              title={search ? "No matching events" : "No events yet"}
              description={search ? "Try a different search term." : "Events will appear here once created."}
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b text-muted-foreground text-xs uppercase tracking-wide">
                      ...
                      <th className="text-left py-3 pr-4 font-medium">Starts</th>
                      <th className="text-left py-3 pr-4 font-medium">Mode</th>
                      <th className="text-right py-3 pr-4 font-medium">RSVPs</th>
                  ...
                  <tbody>
                    {filtered.map((event: IAdminEvent) => (
                      <tr key={event.id} className="border-b last:border-0 hover:bg-muted/40 transition-colors">
                        <td className="py-3 pr-4 max-w-[200px]">
                          <div className="font-medium truncate">{event.name}</div>
                          <div className="text-xs text-muted-foreground truncate">{event.locationName ?? "—"}</div>
                        </td>
                        ...
                        <td className="py-3">
                          <div className="flex items-center gap-1">
                            ...
                            {event.status?.toUpperCase() !== "CANCELLED" && (
                              <Button
                                variant="ghost" size="sm" title="Cancel event"
                                className="text-destructive hover:text-destructive"
                                onClick={() => setCancelTarget({ id: event.id, name: event.name })}
                              >
      ...
      <AlertDialog open={!!cancelTarget} onOpenChange={(open) => { if (!open) { setCancelTarget(null); setCancelReason(""); } }}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Cancel &ldquo;{cancelTarget?.name}&rdquo;?</AlertDialogTitle>
            <AlertDialogDescription>
              This will force-cancel the event. Attendees will be notified. This action cannot be undone.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <div className="space-y-1.5 px-1 py-2">
            <Label htmlFor="cancel-reason">Reason <span className="text-muted-foreground font-normal text-xs">(optional)</span></Label>
            <Textarea id="cancel-reason" placeholder="e.g. Violates platform terms of service" rows={2} value={cancelReason} onChange={(e) => setCancelReason(e.target.value)} />
          </div>
          <AlertDialogFooter>
            <AlertDialogCancel>Go back</AlertDialogCancel>
            <AlertDialogAction className="bg-destructive text-white hover:bg-destructive/90" onClick={handleCancel} disabled={cancelling}>
