...
const StepFive = ({ rewardTiers, updateRewardTier, priceCurrency }: StepFiveProps) => (
  <div className="space-y-4 animate-fade-in">
    <div>
      <h3 className="font-semibold">Reward Tiers</h3>
      <p className="text-sm text-muted-foreground">
        Set a prize for each of the {rewardTiers.length} winner{rewardTiers.length !== 1 ? "s" : ""}.
      </p>
    </div>
          ...
          <CardContent className="p-4 space-y-3">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground">Rank #{tier.rank}</p>
              </div>
            </div>
            ...
            <div className="space-y-1">
              ...
              <ToggleGroup
                type="single"
                value={tier.type}
                onValueChange={(v) => v && updateRewardTier(tier.id, "type", v as RewardType)}
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              {/* Value — label changes by type */}
              <div className="space-y-1 col-span-2">
                <Label className="text-xs">
                  {tier.type === "CASH"   ? `Cash Amount (${priceCurrency})` :
                   tier.type === "POINTS" ? "Points Amount" :
                   tier.type === "BADGE"  ? "Badge Name / ID" :
                   "Value"}
                ...
                <Input
                  type={tier.type === "CASH" || tier.type === "POINTS" ? "number" : "text"}
                  min={1}
                  value={tier.value}
                  onChange={(e) => updateRewardTier(tier.id, "value", e.target.value)}
                  className={cn(
                    "h-9 text-sm",
                    (tier.type === "CASH" || tier.type === "POINTS") &&
                ...
                <>
                  <div className="space-y-1">
                    ...
                    <ToggleGroup
                      ...
                      onValueChange={(v) => v && updateRewardTier(tier.id, "discountType", v as DiscountType)}
                      className="justify-start"
                    >
                      <ToggleGroupItem value="PERCENTAGE" className="rounded-full h-8 text-xs px-3 data-[state=on]:bg-[#531342] data-[state=on]:text-white">
                        %
                      </ToggleGroupItem>
                      <ToggleGroupItem value="FIXED_AMOUNT" className="rounded-full h-8 text-xs px-3 data-[state=on]:bg-[#531342] data-[state=on]:text-white">
                        Fixed
                      </ToggleGroupItem>
                    </ToggleGroup>
                  </div>
                  <div className="space-y-1">
                    ...
                    <Input
                      ...
                      onChange={(e) => updateRewardTier(tier.id, "discountValue", Number(e.target.value))}
                      className="h-9 text-sm"
                      placeholder="20"
                    />
                  </div>
                  <div className="space-y-1">
                    ...
                    <Input
                      ...
                      onChange={(e) => updateRewardTier(tier.id, "usageLimit", Number(e.target.value))}
                      className="h-9 text-sm"
                      placeholder="100"
                    />
                  </div>
              ...
              {/* Quantity — only for non-cash/points types */}
              {tier.type !== "CASH" && tier.type !== "POINTS" && (
                <div className="space-y-1">
                  ...
                  <Input
                    ...
                    min={1}
                    value={tier.quantity}
                    onChange={(e) => updateRewardTier(tier.id, "quantity", Number(e.target.value))}
                    className="h-9 text-sm"
                    placeholder="1"
                  />
                </div>
