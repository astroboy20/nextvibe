...
import { useCommentOnPostcardMutation, useGetPostcardCommentsQuery, useToggleLikePostcardMutation } from "@/app/provider/api/eventApi";
...
function PostcardCard({ postcard }: { postcard: PostcardItem }) {
  ...
  const mediaUrl = postcard.media?.[0]?.mediaUrl ?? "";
  ...
  const handleLike = async () => {
    ...
    setOptimisticCount((c) => (previousLiked ? c - 1 : c + 1));

    try {
      const response = await toggleLikePostcard({
        eventId: postcard.event?.id ?? "",
        postcardId,
      }).unwrap();

      // sync with backend
      setOptimisticLiked(response.liked);
      setOptimisticCount(response.currentLikes);
    } catch {
      // rollback
      setOptimisticLiked(previousLiked);
...
function CommentsPanel({ postcardId }: { postcardId: string }) {
  ...
  const [commentOnPostcard, { isLoading: sending }] =
...
function PeopleList({
  ...
  const handleToggle = async (user: SocialUser) => {
    ...
    const current =
      followingState[user.id] ?? user.isFollowing ?? defaultFollowing;
  ...
  return (
    <div className="space-y-3">
      {users.map((person) => {
        const isFollowing =
          followingState[person.id] ?? person.isFollowing ?? defaultFollowing;
...
const Social = () => {
  ...
  const { data: followingData, isLoading: followingLoading } =
    useGetMyFollowingQuery(undefined, { skip: activeTab !== "people" });
  const { data: followersData, isLoading: followersLoading } =
    useGetMyFollowersQuery(undefined, { skip: activeTab !== "people" });
  const { data: mutualsData, isLoading: mutualsLoading } = useGetMutualsQuery(
    undefined,
    { skip: activeTab !== "people" }
  );
  ...
  const peopleMap = {
    following: {
      users: filterUsers(enrichedFollowing),
      isLoading: followingLoading,
      emptyText: "You're not following anyone yet.",
      defaultFollowing: true,
    },
    followers: {
      users: filterUsers(enrichedFollowers),
      isLoading: followersLoading,
      emptyText: "No followers yet.",
      defaultFollowing: false,
    },
    mutuals: {
      users: filterUsers(mutualUsers),
      isLoading: mutualsLoading,
      emptyText: "No mutuals yet.",
      defaultFollowing: true,
    },
  };
      ...
      <div className="container px-4 py-4">
        ...
        {activeTab === "feed" && (
          <>
            ...
            {!feedLoading && !feedError && postcards.length > 0 && (
              <div className="space-y-4">
                {postcards.map((p) => (
                  <PostcardCard key={p.id ?? p._id ?? p.post_id} postcard={p} />
                ))}
              </div>
            )}
          </>
        ...
        {activeTab === "people" && (
          <div className="space-y-4">
            ...
            <Tabs
              value={peopleTab}
              onValueChange={(v) => setPeopleTab(v as typeof peopleTab)}
            >
              <TabsList className="w-full grid grid-cols-3 h-9">
                ...
                <TabsTrigger value="mutuals" className="text-xs">
                  Mutuals
                </TabsTrigger>
              </TabsList>
            </Tabs>
