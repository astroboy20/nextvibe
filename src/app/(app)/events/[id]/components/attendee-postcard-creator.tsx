...
export function AttendeePostcardLeaderboard({
  ...
  const [phase, setPhase] = useState<PostcardPhase>("pre-event");

  const { data: leaderboardData, isLoading } = useGetPostcardLeaderboardQuery(
    { eventId: eventId ?? "", phase },
    { skip: !eventId },
  );
  ...
  return (
    <Card className="overflow-hidden">
      ...
      <CardContent>
        ...
        <Tabs
          value={phase}
          onValueChange={(v) => setPhase(v as PostcardPhase)}
          className="mb-4"
        >
        ...
        {!isLoading && leaders.length > 0 && (
          <div className="space-y-2">
            {leaders.map((leader: any, index: number) => {
              const name =
                leader.author?.displayName ?? leader.author?.username ?? "User";
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-muted-foreground">
                      ...
                      {(leader.totalComments ?? leader.commentCount ?? 0) !== 1
                        ? "s"
                        : ""}
                    </p>
                  </div>
